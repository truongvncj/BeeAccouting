...
{
    public partial class Buttoanthanhtoan : Form
    {
       ...
       //     txtsotien.Enabled = true;
        ...
        public Buttoanthanhtoan(List<tbl_Socai> dsdinhkhoan, int idtkvat)
        {
            InitializeComponent();
       ...
       //     datechonnam.Value = DateTime.Today.Date;
        ...
        //    LinqtoSQLDataContext dc = new LinqtoSQLDataContext(connection_string);
            var bangvat = (from p in dc.tbl_Bankuploads
                            where p.id == this.idtkvat
                            select p).FirstOrDefault();

            if (bangvat != null)
            {
                bangvat.matchingstatus = true;
                dc.SubmitChanges();
            }
